...
class MemoryBuffer():
    ...
    def _getBatch(self) -> List[Tuple]:
        ...
        S = torch.tensor([self.state_buff[ind].tolist()  for ind in indices])
        A = torch.tensor([self.action_buff[ind].tolist() for ind in indices])
        S_= torch.tensor([self.state_buff2[ind].tolist() for ind in indices])
        R = torch.tensor([self.reward_buff[ind].tolist() for ind in indices])
        T = torch.tensor([self.term_buff[ind].tolist()   for ind in indices])
        return (S, A, S_, R, T)
    ...
    def addMemory(self, state: torch.Tensor, action: int, state2: torch.Tensor, reward: int, terminal: int) -> None:
        ...
        """
        ind: int = random.randint(0, self.buff_size-1) 
        ind = random.randint(0, self.buff_size-1) if self.reward_buff[ind]!=0 else ind
